...
export const capture = tool({
  ...
  args: {
    mode: tool.schema
      .enum(["screen", "window"])
      .optional()
      .describe("screen captures the full desktop (default); window captures the active window"),
  },
  async execute(args) {
    const mode: CaptureMode = args.mode === "window" ? "window" : "screen"
    const socket = socketPath()
    if (!socket || !existsSync(socket)) {
      return "ydotool's private socket is unavailable; ask the user to press PrintScreen (or Alt+PrintScreen) instead."
    }
    if (!existsSync(SCREENSHOT_DIR)) {
      return `Screenshot directory not found: ${SCREENSHOT_DIR}. Ask the user to press PrintScreen once so GNOME creates it.`
    }
    ...
    try {
      ...
      return `Cannot read ${SCREENSHOT_DIR}: ${error instanceof Error ? error.message : String(error)}`
    }

    try {
      await runYdotool(keySequence(mode), socket)
    } catch (error) {
      return `ydotool failed to send the screenshot shortcut: ${error instanceof Error ? error.message : String(error)}`
    }
    ...
    if (created.length === 0) {
      return "No new screenshot appeared; verify the GNOME screenshot shortcut and that ydotool is active, or ask the user to press PrintScreen."
    }
    if (created.length > 1) {
      return `More than one new screenshot appeared (${created.join(", ")}); refusing to guess which to inspect.`
    }
    ...
    try {
      ...
      return `Captured ${created[0]} but could not read it: ${error instanceof Error ? error.message : String(error)}`
    }
    await unlink(filePath).catch(() => undefined)

    if (!isAttachmentSize(bytes.length)) {
      return `Captured ${created[0]} (${Math.round(bytes.length / 1024)} KiB) but it is too large to attach; the file was deleted.`
    }

    const dimensions = pngDimensions(bytes)
    const size = `${Math.round(bytes.length / 1024)} KiB`
    const shape = dimensions ? `${dimensions.width}x${dimensions.height}, ` : ""
    return {
      output: `Captured the ${mode === "window" ? "active window" : "full desktop"} (${shape}${size}). The file was deleted after reading.`,
      attachments: [{ type: "file" as const, mime: "image/png", url: pngDataUri(bytes) }],
    }
  },
})
