...
COMPACTABLE_TYPES = ("message.updated.1", "message.part.updated.1")
...
def db_holder(db_path: str) -> str | None:
    ...
    if fuser:
        ...
        try:
            ...
            pids = "".join(c if c.isdigit() or c == " " else " " for c in out).split()
            if pids:
    ...
    try:
        for pid in os.listdir("/proc"):
            ...
            try:
                ...
                if "opencode" in cmd and "opencode-db-maintain" not in cmd:
...
def orphan_event_ids(con: sqlite3.Connection) -> list[str]:
    ...
    return [
        r[0]
        for r in con.execute(
            "SELECT e.id FROM event e LEFT JOIN session s "
    ...
    for typ, key in (
        ...
        for eid, agg, seq, data in rows:
            ...
            if not isinstance(node, dict) or "id" not in node:
                continue
            groups.setdefault(str(node["id"]), []).append((seq, eid, agg))
        for key_id, items in groups.items():
            if len(items) < 2:
                continue
